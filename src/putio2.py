# -*- coding: utf-8 -*-
"""
A python wrapper for put.io APIv2 created by
...
class Client(object):
	...
	def request(self, path, method='GET', params=None, data=None, files=None, headers=None, raw=False, stream=False):
		'''
...
class _File(_BaseResource):
	...
	def download(self, dest='.', headers=None):
		with open(dest + '/' + self.name, 'wb') as f:
			start = time.clock()
			r = self.client.request('/files/%s/download' % self.id, raw=True, headers=headers, stream=True)
			...
			if total_length is None: # no content length header
				...
				for chunk in r.iter_content(1024):
					dl += len(chunk)
					f.write(chunk)
					# Uncomment to see a progress bar while the file is downloading
